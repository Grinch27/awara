...
// TODO(user): Decide whether settings should become a nested site/appearance/advanced/about menu or remain grouped sections.
// TODO(agent): Keep network under site, appearance/player under look, and data-only actions under advanced.
...
private val settingRootSections = listOf(
    SettingSection.SITE,
...
fun SettingPage(vm: SettingVM = koinViewModel()) {
    ...
    val currentSection = currentSectionName?.let(SettingSection::valueOf)
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    Text(stringResource(currentSection?.titleRes ?: R.string.setting_title))
                },
                navigationIcon = {
                    BackButton(
                        onClick = {
        ...
        LazyColumn(
            ...
            when (currentSection) {
                null -> {
                    settingRootSections.forEach { section ->
                        item {
                            SettingLinkItem(
                                title = {
